...
namespace TestUtils
{
    public static class TestUtils
    {
        ...
        public static List<int> Range(int start, int end) => new List<int>().Build(result => { while(start <= end) result.Add(start++); });

        public static T Build<T>(this T t, Action<T> action) { action.Invoke(t); return t; }
        ...
        {
            bool IsNullableType(Type type, string typeName) => type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>) && type.GetGenericArguments()[0].Name == typeName;
            bool IsNullableEnum(Type type) => type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>) && type.GetGenericArguments()[0].IsEnum;
            return propertyType switch
            {
                ...
                { } type when IsNullableEnum(type) => Enum.GetValues(propertyType.GetGenericArguments()[0]).GetValue(RandomInt(0, Enum.GetValues(propertyType.GetGenericArguments()[0]).Length)),
                _ => null
            };
        }
    }
